...
interface Column {
  ...
  accessor: string; // Key from data to access its value
}

interface StyledTableProps {
  columns: Column[]; // Array of columns
  data: Record<string, any>[]; // Array of data objects
  onSelectionChange?: (selectedData: Record<string, any>[]) => void; // Callback for selecting rows
  onRowClick?: (rowData: Record<string, any>) => void;
}

const StyledTable: React.FC<StyledTableProps> = ({ columns, data, onSelectionChange,onRowClick }) => {
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            {columns.map((column) => (
              <TableCell key={column.header} sx={{ bgcolor: "#f1f4f7", cursor: 'pointer' }} onClick={() => handleSort(column.accessor)}>
        ...
        <TableBody>
          {sortedData.map((row, rowIndex) => (
            <TableRow key={rowIndex}>
              ...
              {columns.map((column) => (
                <TableCell sx={{ fontFamily: "Poppins, sans-serif", color: "#4E5A6A" }} key={column.accessor}>
